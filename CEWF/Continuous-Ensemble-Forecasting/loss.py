...
class WGCLoss:
    ...
    def __call__(self, net, images, class_labels=None, time_labels=None):
        ...
        # from claude: your loss call is net(y + n, sigma, class_labels) with no time_labels argument.
        # time_labels is the normalized lead time; without it the network has no way to learn lead-time-specific denoising.
        # from claude: your loss call is net(y + n, sigma, class_labels) with no time_labels argument.
        # time_labels is the normalized lead time; without it the network has no way to learn lead-time-specific denoising.
        D_yn = net(y + n, sigma, class_labels, time_labels)
        loss = self.area_weights * weight * ((D_yn - y) ** 2)

        # from claude: you have no per-lead residual scaling. Without this, long-lead samples (larger residuals) dominate the loss
        # and short leads train poorly. Precompute the per-lead std for SQG once and divide by it here.

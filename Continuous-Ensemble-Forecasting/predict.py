...
# from claude: you use a stochastic sampler with churn. Martin uses the deterministic Heun sampler exclusively for
# direct forecasting. All ensemble spread comes from different initial latents (get_latents below), not from stochasticity in the ODE.
...
# from claude: you have no get_latents equivalent. This is the entire source of ensemble spread in direct forecasting.
# Each member gets a different draw; alpha controls temporal correlation across lead times (alpha=1: same noise at all leads,
# alpha=0: independent noise per lead). You need to port this function to your SQG forecasting script.
